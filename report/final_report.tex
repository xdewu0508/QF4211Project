...
The canonical raw hourly files span approximately five years, from January 2021 to December 2025. After forward-return alignment, the main notebook works with \texttt{43,808} usable hourly observations per asset. This window contains multiple market phases, including the 2021 bull market, the 2022 drawdown, the 2023 recovery, the 2024 ETF-driven institutional surge, and the volatile conditions late in 2025. To test whether the late-sample environment materially changes the conclusions, we also rerun the same workflow on a shifted 2020--2024 window in a sandbox environment and compare the results in Section~5 and Appendix~\ref{app:regime}. That extra rerun strengthens the regime-sensitivity discussion, but it still remains a fixed-window comparison rather than full walk-forward validation.
...
Each asset is divided chronologically into \texttt{50\%} training, \texttt{25\%} validation, and \texttt{25\%} testing. Under the main 2021--2025 notebook logic this yields \texttt{21,902} training rows, \texttt{10,950} validation rows, and \texttt{10,952} test rows per asset after the forward-return alignment and split-boundary trimming are applied. The same split protocol is then reapplied to the shifted 2020--2024 rerun so that the robustness comparison changes the window, not the design. This avoids look-ahead bias and provides two clean held-out windows under one common protocol. However, the evidence should still be interpreted as fixed-window out-of-sample performance rather than as proof that the same model rankings will persist under rolling or walk-forward re-estimation across different market regimes.
...
To avoid crowding the main discussion, the equity-curve figures are moved to Appendix~\ref{app:figures}, and the regime-comparison figures are collected in Appendix~\ref{app:regime}. The appendix figure sets have distinct roles. The main-window equity curves show the asset-by-asset shape of the canonical 2021--2025 backtests, always comparing buy-and-hold with the best feasible configuration per model. The Appendix C figures then summarize the cross-window regime comparison in aggregate.

The long-only curves in Appendix~\ref{app:fig_long_only} show four related stories. In BTC and XRP, buy-and-hold retains the strongest trend capture while most active strategies remain flatter after costs; in ETH, selected active models narrow the gap but do not sustain clear dominance; in SOL, active strategies can occasionally compete in a harder asset, but the paths remain uneven and model dispersion is wider. The long-short curves in Appendix~\ref{app:fig_long_short} are noisier in all four assets: BTC and ETH illustrate strong cost drag and flatter post-fee trajectories, while SOL and XRP display episodic bursts that are not stable enough to overturn the summary-table ranking. The short-only curves in Appendix~\ref{app:fig_short_only} are the clearest visual confirmation of the earlier statistical result. Across BTC, ETH, SOL, and XRP, the short-only best-per-model paths fail to accumulate wealth consistently after costs, and several decay steadily. The Appendix C comparison figures then show the same story in aggregate: Figure~\ref{fig:appendix_c_strategy_mode} shows the strategy-mode shift, Figure~\ref{fig:appendix_c_model_family} shows the model-family shift, and Figure~\ref{fig:appendix_c_buyhold} shows that passive exposure also becomes substantially stronger in the earlier window.

\section{Conclusion, Limitations, and Future Work}

\subsection{Conclusion}

This study asks: To what extent do ML-based trading strategies outperform traditional rule-based strategies and passive buy-and-hold benchmarks in terms of the Sharpe Ratio, once transaction costs and backtesting constraints are applied to major assets such as Bitcoin (BTC), Ethereum (ETH), Ripple (XRP), and Solana (SOL)? Within the canonical 2021--2025 evaluation window, the answer is: only to a limited extent, and not in any broad or robust sense. Machine-learning models can improve on simple rule-based strategies in selected settings, especially before costs, but they do not consistently or statistically reliably outperform buy-and-hold once transaction costs are applied.
...
Several limitations materially qualify the findings. First, even with the additional 2020--2024 rerun, the study still relies on fixed-window comparisons rather than rolling or expanding walk-forward retraining. The second window improves the regime-sensitivity evidence, but it does not establish whether the observed rankings would persist under continuously updated historical partitions. In a non-stationary market such as cryptocurrency, this remains an important limitation.
